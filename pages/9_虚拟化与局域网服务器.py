...
st.write("不少个人/家庭服务器用户选择部署Nextcloud、Onlyoffice和Codeserver等工具实现浏览器上的文件编辑，我没有考虑部署这类应用，是因为文件编辑以电脑为主，在可以挂载文件系统的情况下在客户机运行编辑软件即可。有复杂的多用户需求由于可以使用gitlab等工具，也不一定要使用这类软件。在文档类型中，如果经常编辑，有软件汇总的体验好一些，但刚需这类软件的还是使用平板电脑等无法挂载服务端文件系统且因没有复杂的多用户需求无需部署gitlab的情况。")
st.write("由于Vivaldi的rss缺少同步功能，部署了FreshRSS用于实现持续接收和局域网同步，另外由于是网页形式，可以正常使用浏览器插件。")