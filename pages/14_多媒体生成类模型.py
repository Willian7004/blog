...
st.write("考虑光照和速度和分辨率优势，开源绘画模型中Sana_1600M_2kpx比较实用，添加device_map=balanced和vae tiling后可以用8g显存生成2.5k图片，但在comfyui中除了只兼容1024px版本的sana_lowvram节点外均不支持类似优化，因此推荐另外部署。sana1.5由于与diffusers库的dpm不一致，无法在通过diffusers库使用相同优化且没有发布高分辨率版本。如果能接受更低的速度和分辨率，使用Hunyuan Video进行图片生成可用1600x896分辨率，结构和光照表现更好。")
