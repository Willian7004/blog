...
st.write("开源绘画模型中cogview4比较有优势，支持横屏和竖屏1080p，光照表现较好，除了不能用于更高分辨率外基本结合了其它开源绘画模型的有点。存在的问题是目前只能通过diffusers库推理（如果sequential offload支持没有合并可以在pr找到），速度较慢，用时为通过comfyui运行flux.1 dev的2到3倍。")
