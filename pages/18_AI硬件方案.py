...
st.write("随着模型小型化和显存优化的发展，12g显存的显卡足以运行Deepcoder 14b和Wan2.1，绘画模型和其它类型的模型需要的显存更少，b580 12g是算力性价比最高的方案。如果进一步考虑Fast Hunyuan等步数蒸馏模型以及即将推出的Qwen3系列，考虑bf16支持的情况下3050 8g即可入门，进一步降低了门槛。")
...
st.write("嵌入式设备中，通常使用NPU运行AI应用。瑞芯微的低端型号主要支持RNN，对LLM支持较好的有RK3576和RK3588，官方以及一些产品提供较为完整的资料。地平线旭日系列资料也较为完整但GPU设计不适合桌面应用，偏向边缘计算。这两种由于只有嵌入式生态，建议只使用官方支持的模型。")
st.write("对于价格更高的香橙派aipro和英伟达jetson，由于价格较高，功耗优势无法抵消算力性价比差距，不如使用独显。在较低的价位，支持avx512的酷睿10代和11代低压cpu也对价格高于rk3576的arm设备造成冲击。对avx2的支持则是在4代到6代酷睿之间完成，可能也对arm处理器的选择有影响。")
