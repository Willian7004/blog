...
st.write("其它开源绘画模型中cogview4总体效果较好，支持横屏和竖屏1080p，光照和细节表现较好，人物结构表现一般。目前只能通过diffusers库推理，速度较慢，使用3070m开启sequential cpu offload显存占用1.4g，推理时间8分钟。如果用于对光照要求不高的风格，Flux.1仍然是效果最好的模型。")
...
st.write("开源视频模型中，虽然此前不少模型运行成本也较高，但Controlnet的作者改进了Hunyuan Video图生视频版本的架构，推出了FramePack，使用6g显存即可实现任意长度推理但实用长度约为1分钟，主流配置下单帧生成时间在10秒内。")
